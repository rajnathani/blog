...
exports.get = function (req, res) {
    ...
    var query = af.xvalidate('query', 'query', {size: [1, 100]});

    if (!af.isValid(query)) {
        return res.send(403, 'Sorry');
    }

    var parsed_keywords = makeKeywords(query);
    fetchResults(parsed_keywords.other_keywords, parsed_keywords.last_keyword,
        30, function (err, results) {
            if (err) return res.send(403, 'Sorry');
            return res.render('search', {results: results, query: query});
        })
...
function fetchResults(keywords, last_keyword, max, callback) {
    etc.startMongoDB('SearchIndex', function (err, SearchIndex, db) {
        ...
        if (last_keyword) {
            ...
            match_dict = {_id: {$in: keywords}};
        }

        SearchIndex.find(match_dict).toArray(function (err, results) {
                if (err) {
                    db.close();
                    return callback(err, null);
                }
...
function mergeResults(results, max) {
    ...
    var sorted = _.sortBy(mapped,function (tup) {
        return tup[2]
    }).reverse();
